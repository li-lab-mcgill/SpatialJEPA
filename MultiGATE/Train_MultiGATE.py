...
def train_MultiGATE(adata1, adata2, hidden_dims=[512, 30], bp_width=450, temp=1.0, type='ATAC', n_epochs=500, lr=0.0001, key_added='MultiGATE',
    ...
    G_df[['Cell1', 'Cell2']] = G_df[['Cell1', 'Cell2']].astype(int)
    ...
    G_gp_df['Peak'] = G_gp_df['Peak'].map(peaks_id_tran)
    G_gp_df = G_gp_df.dropna(subset=['Gene', 'Peak']).copy()
    if G_gp_df.empty:
        raise ValueError('gene_peak_Net does not overlap with selected RNA/ATAC features.')

    G_gp_df['Gene'] = G_gp_df['Gene'].astype(int)
    G_gp_df['Peak'] = G_gp_df['Peak'].astype(int) + adata_Vars1.n_vars

    if type == 'ATAC':
        dist = G_gp_df['Distance'].astype(float)
        ...
        G_gp = sp.coo_matrix(
            (
                weights,
                (
                    np.concatenate((G_gp_df['Gene'], G_gp_df['Peak']), axis=0),
                    np.concatenate((G_gp_df['Peak'], G_gp_df['Gene']), axis=0),
                ),
            ),
        ...
        weights = np.ones(G_gp_df.shape[0] * 2) * protein_value
        G_gp = sp.coo_matrix(
            (
                weights,
                (
    ...
    trainer = MultiGATE(
        hidden_dims1=[X1.shape[1]] + hidden_dims,
        hidden_dims2=[X2.shape[1]] + hidden_dims,
        spot_num=X1.shape[0],
        temp=temp,
        n_epochs=n_epochs,
        lr=lr,
    ...
    embeddings_RNA, embeddings_ATAC, attentions1, attentions2, attentions_gp, loss, ReX_RNA, ReX_ATAC = trainer.infer(
        G_tf,
        G_tf,
        G_gp_tf,
        X1,
    ...
    adata1.obsm[key_added + '_clip_all'] = (
        norm2.fit_transform(cell_reps1.loc[adata1.obs_names, ].values)
        + norm2.fit_transform(cell_reps2.loc[adata2.obs_names, ].values)
    ) / 2.0
    adata2.obsm[key_added + '_clip_all'] = (
        norm2.fit_transform(cell_reps1.loc[adata1.obs_names, ].values)
        + norm2.fit_transform(cell_reps2.loc[adata2.obs_names, ].values)
    ) / 2.0
    ...
    if return_trainer:
        return adata1, adata2, trainer

    return adata1, adata2


def forward_MultiGATE(adata1, adata2, trainer, bp_width=450, type='ATAC', key_added='MultiGATE',
                      verbose=False, save_attention=False, save_reconstrction=False, protein_value=0.001):
    if trainer is None:
        raise ValueError("trainer is required. Call train_MultiGATE(..., return_trainer=True) first.")
    ...
    if verbose:
        ...
        print('Size of Input of atac: ', adata_Vars2.shape)
    '''
    expected_spot_num = int(trainer.mgate.vgp0.shape[0])
    if X1.shape[0] != expected_spot_num:
        raise ValueError(
            "Target spot count ({}) does not match trained model spot count ({}).".format(
                X1.shape[0], expected_spot_num
            )
        )
    '''

    expected_rna_features = int(trainer.mgate.W1[0].shape[0])
    expected_atac_features = int(trainer.mgate.W2[0].shape[0])
    if X1.shape[1] != expected_rna_features or X2.shape[1] != expected_atac_features:
        raise ValueError(
            "Target feature dimensions do not match trained model dimensions: "
            "RNA {} vs {}, ATAC {} vs {}.".format(
                X1.shape[1], expected_rna_features, X2.shape[1], expected_atac_features
            )
        )
    ...
    if type in ['ATAC', 'ATAC_RNA']:
        dist = G_gp_df['Distance'].astype(float)
        gp_bp_width = bp_width if type == 'ATAC' else 2000
...
def prepare_graph_data(adj):
    # Adapted from preprocess_adj_bias (keeps existing index ordering contract).
